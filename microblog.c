...
{
  return set_http_rep(HTTP_VERSION, S_OK, cnt_type, cnt, cnt_len,
                      rep_buff);
}
...
static int set_http_rep_400(char *rep_buff)
{
  return set_http_rep(HTTP_VERSION, S_BAD_REQUEST, TEXT_PLAIN,
                      S_BAD_REQUEST_MSG, strlen(S_BAD_REQUEST_MSG),
                      rep_buff);
}
                             ...
                             char *http_req_res_buff)
{
  ...
  while (!isspace(*req_buff_it) && *req_buff_it != '\0' && req_buff_cnt-- > 0 &&
         req_res_buff_cnt-- > 0)
  {
...
static int set_post_idx(size_t *post_idx, const char *http_req_res_buff)
{
  ...
  if (post_idx_long > SIZE_T_MAX ||
    ...
    return SP_OVERFLOW;
  if (post_idx_long < INT_MIN || (errno == ERANGE && post_idx_long == LONG_MIN))
    return SP_UNDERFLOW;
  if (*end != '\0')
    return SP_INCONV;
  if (post_idx_long < 0)
...
// TODO: keep our server data in this struct
...
static int server_receive(int client_sock_fd, char *req_buff)
{
  int b_req = 0;
  int tot_b_req = 0;
  while ((b_req = recv(client_sock_fd, &req_buff[tot_b_req], REQ_SIZE - tot_b_req, 0)) > 0)
  {
    ...
    tot_b_req += b_req; 
    /* HTTP Requst is sent */
    if (http_req_is_final(req_buff, tot_b_req))
      break; 
    /* req_buff overflows */
    if (tot_b_req >= REQ_SIZE)
      return SR_READ_OVERFLOW; 
  }
  return tot_b_req;
}

enum server_send_errno
{
  SS_ERROR = -1
};
...
void server_proc_req(int client_sock_fd)
{
  ...
  else if (http_req_is_get(req_buff))
  {
    ...
    if (http_req_is_home(req_buff)) 
    {
      ...
      rep_status = set_http_rep_200(posts[0].content_type, posts[0].body, strlen(posts[0].body) + 1, 
        rep_buff);
    }
    else
    {
      ...
      else
      {
        ...
        rep_status = set_http_rep_200(post.content_type, post.body, strlen(post.body) + 1,
                         rep_buff);
      }
    }
  }
